...
export function AttendanceSummary({ eventId }: { eventId: number }) {
    ...
    const participants = useMemo(() => {
        ...
        const sortedReports = [...data.participants].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        if (searchQuery.trim() !== "") {
            ...
            return sortedReports.filter((participant) => {
                ...
                return fullName.includes(lowerQuery) || status.includes(lowerQuery) || checkInDate.includes(lowerQuery);
            });
        }
