...
export function KpiCard({ label, value, icon, trend, trendUp, bgColor, cardBg, comparisonLabel }: KpiCardProps) {
    return (
        <Card
            className={cn(
                "relative overflow-hidden border-2 border-[#5C5C5C] shadow-[4px_4px_0px_0px_rgba(87,66,114,1)]",
                bgColor,
            )}
        >
            {cardBg && (
            ...
            <CardContent className="relative flex flex-col gap-2 z-10 px-6 py-2 bg-blue">
                ...
                <div
                    className={cn(
                        "flex items-center text-sm font-medium font-display",
                        trendUp ? "text-[#003F1F]" : "text-[#820006]",
                    )}
                >
