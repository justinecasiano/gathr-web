...
export function GlobalSearch() {
    ...
    const handleSearch = (val: string) => {
        ...
        if (val.length > 0) {
            ...
            const filtered = items.filter((item) => {
                ...
                const matchesContext = isFilteringPage ? item.category === "event" : true;
    ...
    const dynamicPlaceholder = useMemo(() => {
        const match = Object.keys(PATH_PLACEHOLDERS).find((path) => pathname.endsWith(path));
    ...
    return (
        <div className="relative w-full px-5" ref={searchRef}>
            ...
            {isOpen && results.length > 0 && (
                <div className="absolute top-full left-5 right-5 mt-4 bg-white border-2 border-[#5C5C5C] rounded-xl shadow-[8px_8px_0px_0px_rgba(87,66,114,1)] z-50 overflow-hidden">
                    <ul ref={resultsListRef} className="max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-[#7B55A3]">
