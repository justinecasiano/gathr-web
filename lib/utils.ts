...
export const generateOrganizerDashboardAnalytics = (events: BaseEvent[], dateRange: DateRange | undefined) => {
    ...
    const calcStats = (eventList: BaseEvent[]) => {
        ...
        const totalRegistered = eventList.reduce((sum, e) =>
            sum + (e.participants?.[0]?.count ?? 0), 0
        );
        const totalPresent = eventList.reduce((sum, e) =>
            sum + (e.present_count?.[0]?.count ?? 0), 0
        );
        const totalResponses = eventList.reduce((sum, e) =>
            sum + (e.response_count?.[0]?.count ?? 0), 0
        );
    ...
    const feedbackTrendData = last6Months.map(monthDate => {
        ...
        const monthAvg = totalResponsesInMonth > 0
            ? (totalWeightedRating / totalResponsesInMonth)
            : 0;
