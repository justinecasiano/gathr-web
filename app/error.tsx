...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
    ...
    return (
        <main className="relative flex min-h-screen w-full items-start sm:items-center justify-center bg-brand-dark p-6 overflow-hidden">
            <div className="max-w-4xl w-full flex flex-col md:flex-row gap-8 items-center z-20 mt-[15vh] sm:mt-0">
                <div className="relative h-64 w-64 md:h-100 md:w-100 shrink-0 hidden sm:block">
                    <Image src="/svgs/error-image.svg" alt="Error Illustration" fill className="object-contain" priority />
                </div>
                ...
                <div className="relative flex flex-col justify-center items-center md:items-start text-center md:text-left w-full">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm sm:max-w-none justify-center md:justify-start">
                        <Button
                            type="button"
                            onClick={() => reset()}
                            variant="elevated"
