...
import {cn, generateModeratorDashboardAnalytics, generateOrganizerDashboardAnalytics} from "@/lib/utils";
...
import {useModeratorEvents} from "@/hooks/use-moderator-events";
...
export default function DashboardPage() {
    ...
    const {data: rawEvents, isLoading: isEventsLoading} = useModeratorEvents({from: fetchFrom, to: activeTo});

    const analytics = useMemo(() => {
        if (!rawEvents) return null;
        return generateModeratorDashboardAnalytics(rawEvents, dateRange);
    }, [rawEvents, dateRange]);
    ...
    return (
        <div className="flex relative min-h-screen w-full flex-col bg-[#F7F0FF] overflow-hidden">
            ...
            <main className="flex-1 px-10 py-6 space-y-8 max-w-[1600px] mx-auto w-full z-40">
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                    ...
                    <Card
                        ...
                        <CardHeader className="flex flex-row items-center gap-3 px-0 pt-0">
                            ...
                            <CardTitle className="text-xl font-bold font-display text-[#261A36]">Event
                                Status</CardTitle>
                        </CardHeader>
                            ...
                            {pieData.map((status) => (
                                <div key={status.name} className="flex items-center justify-between mx-auto w-[70%]">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span
                                            className="text-base font-normal font-display text-black">{status.name}</span>
                                    </div>
                ...
                <Card
                    className="border-2 border-[#5C5C5C] shadow-[8px_8px_0px_0px_rgba(87,66,114,1)] rounded-2xl p-6 mb-5">
                    <CardHeader className="flex flex-row items-center gap-3 px-0 pt-0">
                        <Image src="/svgs/average-feedback-icon.svg" width="25" height="25" alt="Icon"/>
                        <CardTitle className="text-xl font-bold font-display text-[#261A36]">
                            Average Feedback Rating For All Events
                        </CardTitle>
                    </CardHeader>
