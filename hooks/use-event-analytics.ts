...
export function useEventAnalytics(eventId: number) {
    return useQuery({
        ...
        queryFn: async (): Promise<EventAnalytics> => {
            ...
            const summary = {
                answered: participants.length > 0 ? participants.filter((p) => p.response_status === "ANSWERED").length : 0,
                noResponse:
                    participants.length > 0 ? participants.filter((p) => p.response_status === "NO_RESPONSE").length : 0,
                absent: participants.length > 0 ? participants.filter((p) => p.response_status === "ABSENT").length : 0,
            };
            ...
            const questionsAnalytics: QuestionAnalytics[] = (form.questions as FeedbackQuestion[]).map((q) => {
                ...
                if (q.type === "slider") {
                    ...
                    analytics.sliderData = [1, 2, 3, 4, 5].map((val): SliderSummary => {
                        const count = totalResponses > 0 ? allAnswers.filter((ans) => Number(ans) === val).length : 0;
                ...
                if (q.type === "text_input") {
                    analytics.textAnswers =
                        totalResponses > 0
                            ? allAnswers.filter((ans): ans is string => typeof ans === "string").map((ans) => ans)
                            : [];
                }
